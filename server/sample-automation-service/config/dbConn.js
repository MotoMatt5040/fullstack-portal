// Sample Automation Service Database Connection
...
const withDbConnection = async ({
  ...
  allowAbort = false,
  allowRetry = false,
  database = 'promark', // default to promark for Sample Automation service
}) => {
  const pool = database === 'fajita' ? await getFajitaPool() : await getPromarkPool();
...
module.exports.getFajitaPool = getFajitaPool;
